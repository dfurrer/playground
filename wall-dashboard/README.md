...
- **Weather** — current conditions, high/low, and a 5-hour strip, from the
  free [Open-Meteo](https://open-meteo.com) API (no key required).
...
3. Tap the gear icon and set your location (type a city, or tap **Use GPS**
   once and allow the permission prompt).
4. Add it to the home screen: browser menu → **Add to Home screen** →
   **Install**. This makes it launch full-screen (no browser chrome) via the
   PWA manifest.
5. Launch it from the home screen icon, then mount the phone in its wall
   dock/case in portrait orientation.
6. In Android **Settings → Display → Screen timeout**, set it to the longest
   ...
   visible tab.
7. Optional but recommended for a dedicated wall panel: install a kiosk
